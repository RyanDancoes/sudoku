...
def solver(options):
    ...
    while True:
        ...
        if cells and value and remove:
            print(f"Locked candidate of {value} at: ", end='')
            for cell in cells:
            ...
            print()
            break
        cells, value, remove = techniques2.check_everything(options, 'p_t')
        if cells and value and remove:
            print(f"Pointing tuple with value {value} at: ", end='')
            for cell in cells:
                print(f"{cell[0]+1}{cell[1]+1}", end='     ')
            print()
            print(f"Cross off {value} at: ", end='')
...
def load():
    ...
    #     board.append(row)
    # input(f'{board}')
    board = [[0, 7, 0, 4, 0, 0, 0, 0, 0], [0, 9, 0, 0, 0, 6, 0, 0, 0], [3, 0, 2, 0, 1, 0, 0, 0, 6], [4, 0, 5, 0, 0, 1, 0, 0, 0], [8, 0, 1, 0, 0, 0, 4, 0, 7], [0, 2, 0, 0, 3, 4, 6, 0, 1], [0, 1, 0, 0, 0, 0, 7, 0, 8], [0, 0, 0, 1, 0, 0, 2, 6, 0], [0, 0, 0, 0, 5, 7, 0, 1, 0]]
